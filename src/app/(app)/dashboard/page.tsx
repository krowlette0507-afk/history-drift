...
function StatCard({ icon, value, label, color }: {
  ...
  return (
    <div className="rounded-2xl p-3 flex flex-col items-center gap-1 text-center md:flex-row md:items-center md:gap-4 md:text-left md:p-4"
      ...
      <div>
        <div className="text-xl md:text-2xl font-serif font-bold text-amber-200 leading-none">{value}</div>
        <div className="text-[10px] md:text-xs font-sans text-amber-800/60 mt-0.5 leading-tight">{label}</div>
      </div>
    </div>
  );
}
...
function SessionCard({ session }: { session: StoredSession }) {
  ...
  return (
    <Link href={`/interview?interviewer=${session.interviewerId}`}
      className="group flex items-center gap-3 p-3 md:p-4 rounded-2xl border transition-all hover:scale-[1.01]"
      style={{ background: "rgba(18,11,4,0.7)", borderColor: isComplete ? "rgba(90,52,20,0.3)" : `${iv.accentColor}30` }}>
      <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center font-serif font-bold text-sm text-amber-100"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] font-sans text-amber-800/50 mt-0.5">
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen overflow-y-auto px-4 py-6 md:px-8 md:py-8">

      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="text-amber-600/50 text-xs font-sans uppercase tracking-[0.3em] mb-1">Welcome back</div>
          <h1 className="text-amber-200 font-serif font-bold text-2xl md:text-3xl">Your Story</h1>
          <p className="text-amber-700/50 font-serif italic text-xs md:text-sm mt-1">
            {hasContent
              ? `${stats.wordCount.toLocaleString()} words · ${stats.sessionCount} session${stats.sessionCount !== 1 ? "s" : ""}`
              : "Your life story begins with a single conversation."}
          </p>
        </div>
        <Link href="/interview"
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-serif font-semibold text-sm text-amber-50 transition-all hover:scale-[1.02] flex-shrink-0"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <StatCard icon={<Mic size={16} />} value={stats.sessionCount} label="Interview Sessions" color="#c8843a" />
        <StatCard icon={<Users size={16} />} value={stats.peopleCount} label="People Captured" color="#c84a4a" />
        <StatCard icon={<MapPin size={16} />} value={stats.placesCount} label="Places Remembered" color="#3a8a4a" />
        <StatCard icon={<Lightbulb size={16} />} value={stats.lessonsCount} label="Life Lessons" color="#8a7a2a" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-amber-300/80 font-serif font-semibold text-base md:text-lg">Interview Sessions</h2>
            ...
            {sessions.length === 0 ? (
              <div className="rounded-2xl p-6 text-center border border-dashed border-amber-900/30">
                <Mic size={28} className="text-amber-800/30 mx-auto mb-3" />
                <p className="text-amber-700/50 font-serif italic text-sm mb-4">
                  No interviews yet. Your story is waiting to be told.
                </p>
                <Link href="/interview"
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-serif text-amber-200 transition-all hover:scale-105"
          ...
          {/* Explore grid — 2-col on mobile, 3-col on desktop */}
          <div>
            <h2 className="text-amber-300/80 font-serif font-semibold text-base md:text-lg mb-3">Explore Your Story</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                { href: "/timeline", icon: <Calendar size={15} />, label: "Timeline", desc: `${stats.eventsCount} events`, color: "#2a7a8a" },
                { href: "/people", icon: <Users size={15} />, label: "People", desc: `${stats.peopleCount} captured`, color: "#c84a4a" },
                { href: "/places", icon: <MapPin size={15} />, label: "Places", desc: `${stats.placesCount} places`, color: "#3a8a4a" },
                { href: "/biography", icon: <BookOpen size={15} />, label: "Biography", desc: "Generate story", color: "#7a4a20" },
                { href: "/lessons", icon: <Lightbulb size={15} />, label: "Life Lessons", desc: `${stats.lessonsCount} lessons`, color: "#8a7a2a" },
                { href: "/ask", icon: <Sparkles size={15} />, label: "Ask Me Anything", desc: "Q&A from your story", color: "#5a5a8a" },
              ].map((item) => (
                <Link key={item.href} href={item.href}
                  className="group flex items-center gap-3 p-3 md:p-4 rounded-2xl border border-amber-900/20 transition-all hover:border-amber-800/40 hover:scale-[1.02]"
                  ...
                  <div className="min-w-0">
                    <div className="text-xs md:text-sm font-serif text-amber-300/80 font-semibold group-hover:text-amber-200 transition-colors truncate">{item.label}</div>
                    <div className="text-[9px] md:text-[10px] font-sans text-amber-800/50 mt-0.5 truncate">{item.desc}</div>
                  </div>
                </Link>
              ))}
            </div>
        ...
        {/* Right sidebar — full width on mobile, shown below content */}
